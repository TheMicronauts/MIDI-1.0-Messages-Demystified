...
I encourage developers to approach MIDI with this mindset if they want to avoid writing wonky programs that are hostile to musical practice and culture. It means, among other things, not trying to replace MIDI, not hiding it under too many layers of abstraction, not converting it—with all the errors caused by bugs and rounding—but instead passing it through as is, without changing the order of the messages—changing the order of the messages, in particular, amounts to rewriting the score, which is obviously unacceptable (I’m looking at you VST3).
